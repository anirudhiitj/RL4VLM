...
def collect_task_rollout(actor_critic, envs, rollouts, args, tokenizer, prompt_template_fn,
    ...
    """
    Collect a full rollout for a specific task (card configuration).

    Args:
        task_cards: Tuple of 4 ints — the card values for this task.
    """
    # Reset env with specific cards via proper wrapper chain
    # set_options propagates through VecPyTorchFrameStack -> VecPyTorch -> DummyVecEnv -> Monitor -> Points24
    envs.set_options({"cards_num": list(task_cards)})
    obs = envs.reset()

    # Get infos from the underlying env for prompt construction
    raw_env = envs.envs[0] if hasattr(envs, 'envs') else envs.venv.envs[0]
    underlying = raw_env
    while hasattr(underlying, 'env'):
        underlying = underlying.env
    infos = [{"Cards": underlying.cards, "Numbers": underlying.cards_num, "Formula": []}]
