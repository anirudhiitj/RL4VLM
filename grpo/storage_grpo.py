...
class GRPOStorage:
    ...
    def insert(self, obs, group_output_ids, group_log_probs, group_rewards):
        """
        Insert one state's group data.

        Args:
            obs: (1, *obs_shape) or (*obs_shape)
            group_output_ids: (group_size, max_output_len)
            group_log_probs: (group_size,)
            group_rewards: (group_size,)
        """
    ...
    def feed_forward_generator(self, flat_advantages, mini_batch_size):
        """
        Generator that yields mini-batches for GRPO update.

        All data is flattened from (num_states, group_size) → (num_states * group_size,).
        """
        total_samples = self.num_states * self.group_size

        # Flatten everything
        # obs: repeat each state's obs group_size times
